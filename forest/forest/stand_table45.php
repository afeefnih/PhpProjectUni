...
<head>
    ...
    function calculateProductionVolume($dbc, $group) {
        $query = "SELECT SUM(volume) AS totalVolume FROM newforestori 
    ...
    function calculatTotalDamage515($dbc, $group) {
        $query = "SELECT SUM(damage_crown) AS totalDamageCrown, SUM(damage_stem) AS totalDamageStem FROM newforestori 
                    WHERE spgroup = $group 
                    AND DiameterClass = 1
                    AND blockX = 1 AND blockY = 1";
        $result = mysqli_query($dbc, $query);
    ...
    function calculateGrowth30Volume515($dbc, $group) {
        $query = "SELECT SUM(Growth30) AS totalVolume FROM newforestori 
                  WHERE spgroup = $group 
    ...
    function calculateProduction30Volume530($dbc, $group) {
        $query = "SELECT SUM(Production30) AS totalVolume FROM newforestori 
                  WHERE spgroup = $group 
                  AND Diameter30 <= 5 AND Diameter30 >= 15 
                  AND blockX = 1 AND blockY = 1";
        $result = mysqli_query($dbc, $query);
